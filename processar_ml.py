...
def renomear_colunas_ml(df):
    ...
    for col in df.columns:
        ...
        # CORREÇÃO: Mapear código do anúncio (com e sem acento)
        # Excel ML usa "# de anúncio" (com acento) — normaliza para comparar
...
def processar_arquivo_ml(arquivo, loja, imposto, engine):
    ...
    for idx, row in df.iterrows():
        try:
            ...
            # Filtrar por status (usa normalização de acentos)
            if 'status' in df.columns:
                status_normalizado = _normalizar_texto(str(row['status']))

                # Lista de palavras que indicam venda inválida
                palavras_bloqueio = [
                    'cancelad', 'devolv', 'devoluc',
                    'reembolso', 'mediacao',
                ]
