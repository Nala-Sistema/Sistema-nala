...
Versão: 2.5 (11/03/2026) - RESTAURAÇÃO INTEGRAL DE TODAS AS FUNÇÕES
Integridade: 100% (Sem omissões ou simplificações)
"""
...
def buscar_custos_skus(engine, force_refresh=None):
    """
    Busca custos de SKUs priorizando dim_produtos.preco_a_ser_considerado.
    Contém fallback para soma de componentes se o preço estiver zerado.
    """
    query = """
        SELECT 
            p.sku,
...
def buscar_skus_validos(engine):
    """
    Busca lista de SKUs válidos (ativos) de dim_produtos.
    """
...
# ============================================================
# LOGS, DUPLICATAS E DESCARTES
# ============================================================

def gravar_log_upload(engine, info):
    """
    Grava log de upload com conversão de datas BR para formato banco.
    """
    try:
        ...
        cursor.execute(sql, (
            ...
            periodo_inicio, periodo_fim, info.get('total_linhas'),
            info.get('linhas_importadas'), info.get('linhas_erro'),
...
def buscar_duplicatas_loja(engine, loja):
    """
    Carrega set de (pedido, sku) para evitar reimportação duplicada.
    """
...
def gravar_venda_descartada(cursor, dados):
    """
    Grava log de vendas canceladas/devolvidas para dashboard de perdas.
    """
    sql = """
        INSERT INTO fact_vendas_descartadas (
            data_processamento, marketplace, loja, numero_pedido, 
            status_original, motivo_descarte, receita_estimada, 
            tarifa_venda_estimada, tarifa_envio_estimada, arquivo_origem
        ) VALUES (NOW(), %s, %s, %s, %s, %s, %s, %s, %s, %s)
    """
    try:
        cursor.execute(sql, (
            dados['marketplace'], dados['loja'], dados['numero_pedido'],
            dados['status_original'], dados['motivo_descarte'],
            dados['receita'], dados['tarifa'], dados['frete'], dados['arquivo']
        ))
        return True
    except Exception:
        return False
...
def gravar_venda_pendente(cursor, dados):
    """
    Grava venda pendente (SKU não cadastrado ou divergência).
    """
    sql = """
    ...
    try:
        cursor.execute(sql, (
            ...
            dados.get('quantidade', 1), dados.get('preco_venda', 0),
            dados.get('valor_venda_efetivo', 0), dados.get('imposto', 0),
            dados.get('comissao', 0), dados.get('frete', 0),
            dados.get('tarifa_fixa', 0), dados.get('outros_custos', 0),
            dados.get('total_tarifas', 0), dados.get('valor_liquido', 0),
            dados.get('arquivo_origem', ''), dados.get('motivo', 'SKU não cadastrado')
        ))
        return True
    except Exception:
        return False
...
def buscar_pendentes_resumo(engine):
    ...
    query = """
        SELECT
            sku, COUNT(*) as total_vendas, SUM(valor_venda_efetivo) as receita_total,
            STRING_AGG(DISTINCT marketplace_origem, ', ') as marketplaces,
            STRING_AGG(DISTINCT loja_origem, ', ') as lojas,
            MIN(data_venda) as primeira_venda, MAX(data_venda) as ultima_venda
    ...
    try:
        ...
        st.error(f"Erro no resumo de pendentes: {e}")
        return pd.DataFrame()

def reprocessar_pendentes_por_sku(engine, sku):
    """
    Reprocessa vendas pendentes de um SKU após cadastro.
    Esta função contém a lógica exata de recálculo de margem necessária.
    """
    skus_validos = buscar_skus_validos(engine)
    if sku not in skus_validos:
        return {'sucesso': 0, 'erros': 0, 'mensagem': f"SKU '{sku}' não cadastrado."}
    ...
    if df_pendentes.empty:
        return {'sucesso': 0, 'erros': 0, 'mensagem': "Nenhuma venda pendente."}
    ...
    sql_snapshot = """
        INSERT INTO fact_vendas_snapshot (
            ...
            tarifa_fixa, outros_custos, total_tarifas, valor_liquido,
    ...
    for _, row in df_pendentes.iterrows():
        try:
            ...
            cursor.execute(sql_snapshot, (
                row['marketplace_origem'], row['loja_origem'], row['numero_pedido'],
                row['data_venda'], sku, row['codigo_anuncio'], row['quantidade'],
                row['preco_venda'], receita, custo_unit, custo_total,
                row['imposto'], row['comissao'], row['frete'], row['tarifa_fixa'],
                row['outros_custos'], row['total_tarifas'], row['valor_liquido'],
                margem_total, margem_pct, row['arquivo_origem']
            ))
            ids_repro.append(int(row['id']))
            sucesso += 1
            cursor.execute(f"RELEASE SAVEPOINT repro_{row['id']}")
        except Exception:
    ...
    cursor.close()
    conn.close()
    return {'sucesso': sucesso, 'erros': erros, 'mensagem': f"SKU {sku}: {sucesso} reprocessadas."}

# ============================================================
# ESTRATÉGICO: CURVA ABC E LOGÍSTICA
# ============================================================

def recalcular_curva_abc(engine, dias=30):
    """
    Recalcula Curva ABC (Pareto) dos anúncios na dim_tags_anuncio.
    """
    ...
    try:
        ...
        return {'total_anuncios': len(df), 'a': contagem.get('A', 0), 'b': contagem.get('B', 0), 'c': contagem.get('C', 0)}
    except Exception:
