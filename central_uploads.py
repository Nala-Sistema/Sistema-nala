...
def processar_ml(arquivo, loja, imposto, engine):
    ...
    # Buscar custos (TABELA CORRETA!)
    query = """
        SELECT s.sku, COALESCE(c.custo_total, 0) as custo
        FROM dim_skus s
    ...
    st.success(f"✅ {len(df_result)} vendas!")
    
    col1, col2 = st.columns(2)
    col1.metric("Vendas", len(df_result))
    col2.metric("Receita", f"R$ {df_result['receita'].sum():,.2f}")
